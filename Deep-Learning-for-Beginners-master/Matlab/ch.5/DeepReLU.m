% DeepReLU 函数基于随机梯度下降策略，使用反向传播算法，通过训练数据集对权重进行更新。
% 与前面不同的是，这里在 3 个隐藏层中使用了 ReLU 作为激活函数。
%
...
function [W1, W2, W3, W4] = DeepReLU(W1, W2, W3, W4, X, D)
  ...
  for k = 1:N
    ...
    y1 = ReLU(v1); % 隐藏层 1 的激活值
                   % 20x1 矩阵（20 维向量）

    v2 = W2*y1; % 隐藏层 2 的加权和
                % 20x1 矩阵（20 维向量）（[20x20] x [20x1] = [20x1]）
    y2 = ReLU(v2); % 隐藏层 2 的激活值
                   % 20x1 矩阵（20 维向量）

    v3 = W3*y2; % 隐藏层 3 的加权和
                % 20x1 矩阵（20 维向量）（[20x20] x [20x1] = [20x1]）
    y3 = ReLU(v3); % 隐藏层 3 的激活值
                   % 20x1 矩阵（20 维向量）
    ...
    delta3 = (v3 > 0).*e3; % 隐藏层 3 的 delta，其中 (v3 > 0) 是 ReLU 函数的导数
                           % 20x1 矩阵（20 维向量）

    e2     = W3'*delta3; % 隐藏层 2 的 e
                         % 20x1 矩阵（20 维向量）（[20x20]' x [20x1] = [20x1]）
    delta2 = (v2 > 0).*e2; % 隐藏层 2 的 delta，其中 (v2 > 0) 是 ReLU 函数的导数
                           % 20x1 矩阵（20 维向量）

    e1     = W2'*delta2; % 隐藏层 1 的 e
                         % 20x1 矩阵（20 维向量）（[20x20]' x [20x1] = [20x1]）
    delta1 = (v1 > 0).*e1; % 隐藏层 1 的 delta，其中 (v2 > 0) 是 ReLU 函数的导数
                           % 20x1 矩阵（20 维向量）
